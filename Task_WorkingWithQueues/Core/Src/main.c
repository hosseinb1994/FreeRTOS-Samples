...
int main(void)
{
  ...
  xTaskCreate(SenderTask, "Sender Task",
		  100, NULL, 1, &Sender_handle);

  xTaskCreate(ReceiverTask, "Receiver Task",
  		  100, NULL, 1, &Receiver_handle);
...
void ReceiverTask(void *pvParameters){
	...
	BaseType_t qStatus;
	uint8_t message[] = "The value received id : ";
	uint8_t messageErr[] = "Error: Could not receive.\r\n";
	uint8_t freeline[] = "\r\n";
	char value_str[20];
	...
	while(1){
		...
		if(qStatus == pdPASS){
			...
			itoa(value_received, value_str, 10);

			HAL_UART_Transmit(&huart2, (uint8_t *)value_str, strlen(value_str), 100);
			HAL_UART_Transmit(&huart2, freeline, sizeof(freeline), 100);
		}
		else{
			HAL_UART_Transmit(&huart2, messageErr, sizeof(messageErr)-1, 100);
		}
